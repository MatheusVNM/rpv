...
class Visualizar_passagens_model extends CI_Model
{
    ...
    {
        $sql = "SELECT alocacaointermunicipal.alocacaointermunicipal_id, onibus.onibus_placa, cidade.cidade_nome, 
    ...
    public function getPassagensEspecificas($idCompraPassagem)
    {
        $sql = "SELECT tipo_passagem.tipo_passagem_nome_tipo, COUNT(comprapassagem.comprapassagem_tipo_passagem_id) FROM 
        ...
        alocacaointermunicipal.alocacaointermunicipal_id = 1 GROUP BY tipo_passagem.tipo_passagem_nome_tipo";
        $result = $this->db->query($sql);
        if (!$result) {
            $retorno['success'] = false;
            $retorno['error'] = $this->db->error();
